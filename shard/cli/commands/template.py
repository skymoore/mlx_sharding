    ...
    try:
        ...
        try:
            template_path = hf_hub_download(
                repo_id=model, filename="chat_template.jinja", repo_type="model"
            )
            ...
            click.echo(
                f"  No chat_template.jinja found, checking tokenizer_config.json..."
            )
            try:
                config_path = hf_hub_download(
                    repo_id=model, filename="tokenizer_config.json", repo_type="model"
                )
        ...
        if not chat_template:
            ...
            click.echo(
                f"   Checked: chat_template.jinja and tokenizer_config.json", err=True
            )
        ...
        if show:
            ...
            click.echo(
                f"  Use with: mlx-shard-api --model {model} --chat-template {output}"
            )
...
def show(name: str):
    ...
    if not template_path.exists():
        ...
        click.echo(
            f"   Use 'mlx-shard template list' to see available templates", err=True
        )
        return
...
def validate(file: str):
    ...
    try:
        ...
        for var in recommended_vars:
            ...
            if (
                f"{{{{{var}" not in template_content
                and f"{{% for" not in template_content
            ):
        ...
        try:
            ...
            click.echo(
                f"⚠ Warning: Template failed to render with sample data: {e}", err=True
            )

...
from shard.orchestrator.planner import ShardAssignment
...
class ModelFileDistributor:
    ...
    async def distribute_to_peers(
        ...
        logger.info(
            f"Distribution complete: {successful}/{len(results)} peers successful"
        )

        return results

    async def distribute_to_peer(
        self,
        peer: PeerInfo,
        ...
        """
        logger.info(
            f"Distributing files to peer {peer.id[:8]} @ {peer.host}:{peer.http_port}"
        )
        ...
        try:
            ...
            if self._is_localhost(peer.host):
                logger.info(
                    f"  ✓ Peer is on localhost - using local model files (no transfer needed)"
                )
            ...
            cached_files = await self._check_peer_cache(
                peer, shard.peer_address.split(":")[0]
            )
            ...
            for file_name in required_files:
                ...
                # Transfer file
                logger.info(
                    f"  → {file_name} ({file_path.stat().st_size / (1024**2):.1f}MB)"
                )
            ...
            logger.error(
                f"Error distributing to peer {peer.id[:8]}: {e}", exc_info=True
            )
            return False
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    url, params=params, timeout=aiohttp.ClientTimeout(total=600)
                ) as resp:
                    if resp.status == 200:
                        result = await resp.json()
                        logger.info(
                            f"Cache check successful: {len(result)} files found"
                        )
                        return result
                    else:
            ...
            logger.warning(
                f"Failed to check peer cache at {host}:{peer.http_port}: {type(e).__name__}: {e}"
            )
            return {}
            ...
            async with aiohttp.ClientSession() as session:
                with open(file_path, "rb") as f:
                    for chunk_num in range(total_chunks):
                        ...
                        if progress_callback:
                            progress_callback(
                                peer.id, file_name, "transferring", progress
                            )
            ...
            try:
                with open(hash_cache_file, "r") as f:
                    ...
                    if file_path.name in hash_cache:
                        ...
                        if (
                            cached_entry.get("size") == current_stat.st_size
                            and cached_entry.get("mtime") == current_stat.st_mtime
                        ):
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status == 200:
                        result = await resp.json()
                        logger.info(
                            f"  ✓ Peer configured to use local path: {model_path}"
                        )
                        return True
                    else:
                        logger.warning(
                            f"Failed to set local model path: HTTP {resp.status}"
                        )
                        return False

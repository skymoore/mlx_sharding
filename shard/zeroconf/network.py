...
class NetworkDiscovery:
    ...
    def get_interfaces() -> List[NetworkInterface]:
        ...
        for iface_name, iface_addrs in addrs.items():
            ...
            logger.debug(
                f"Found interface: {iface_name} @ {ipv4_addr} "
    ...
    def find_common_networks(
        local_interfaces: List[NetworkInterface],
        ...
        for local_iface in local_interfaces:
            ...
            for remote_iface in remote_interfaces:
                ...
                if local_net.overlaps(remote_net):
                    ...
                    logger.debug(
                        f"Common network found: {local_iface.name} <-> {remote_iface.name} "
    ...
    def test_connectivity(
        local_ip: str, remote_ip: str, port: int, timeout: float = 2.0
    ) -> Tuple[bool, float]:
        """
    ...
    def select_best_network(
        ...
        for local_iface, remote_iface in common_networks:
            ...
            reachable, latency_ms = NetworkDiscovery.test_connectivity(
                local_iface.ip_address, remote_iface.ip_address, remote_port
            )

            if not reachable:
                logger.debug(
                    f"Path not reachable: {local_iface.ip_address} -> {remote_iface.ip_address}"
                )
                continue
            ...
            logger.info(
                f"Path found: {local_iface.name}({local_iface.ip_address}) -> "
        ...
        logger.info(
            f"✓ Best path selected: {best['local_iface']}({best['local_ip']}) -> "

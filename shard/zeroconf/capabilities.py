...
class SystemCapabilities:
    ...
    def estimate_model_memory(
        model_path_or_repo: str, context_length: int = 8192
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            logger.info(
                f"Model file size: {weights_gb:.1f}GB ({bits}-bit quantization)"
            )
            logger.info(
                f"Model architecture: {num_layers} layers, {num_attention_heads} attn heads, {num_key_value_heads} KV heads"
            )
            ...
            kv_cache_gb = (kv_cache_elements * 2) / (
                1024**3
            ...
            estimates[f"kv_cache_{context_length//1024}k_gb"] = round(
                kv_gb_requested, 2
            )
            estimates[f"total_{context_length//1024}k_gb"] = round(
                weights_gb + kv_gb_requested + activation_overhead_gb, 2
            )

            estimates["activation_overhead_gb"] = round(activation_overhead_gb, 2)

            logger.info(
                f"Model memory estimate: {weights_gb:.1f}GB weights, "
    ...
    def _default_memory_estimate(context_length: int) -> Dict[str, float]:
        ...
        estimates[f"total_{context_length//1024}k_gb"] = round(
            weights_gb + kv_cache_gb + activation_gb, 2
        )

...
class PeerDiscovery:
    ...
    def __init__(self, role: str = "peer", bind_ip: Optional[str] = None):
        ...
        logger.info(
            f"Initialized discovery service (role={role}, id={self.peer_id[:8]})"
        )
        if bind_ip:
    ...
    def discover_peers(self, timeout: float = DISCOVERY_TIMEOUT) -> List[PeerInfo]:
        ...
        for peer in peers:
            logger.info(
                f"  - {peer.id[:8]} @ {peer.address} ({peer.ram_available_gb:.1f}GB RAM)"
            )
    ...
    def _start_mdns_announcement(
        self, grpc_port: int, http_port: int, capabilities: Dict
    ):
    ...
    def _process_udp_announcement(self, message: Dict, host: str):
        ...
        try:
            ...
            if is_new:
                logger.debug(
                    f"Discovered peer via UDP: {peer_id[:8]} @ {peer_info.address}"
                )
...
class _MDNSListener(ServiceListener):
    ...
    def _process_service_info(self, info: ServiceInfo):
        ...
        try:
            ...
            logger.debug(
                f"Discovered peer via mDNS: {peer_id[:8]} @ {peer_info.address}"
            )
